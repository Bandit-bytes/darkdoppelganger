...
public class EnderDaggerEntity extends AbstractMagicProjectile implements IEntityWithComplexSpawn, GeoAnimatable {
    public int delay;
    ...
    public EnderDaggerEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        setNoGravity(true);
    }

    public EnderDaggerEntity(Level level) {
        this(EntityRegistry.ENDER_DAGGER_PROJECTILE.get(), level);
    }
    ...
    private void createEnderField() {
        EnderField enderField = new EnderField(this.level());
        enderField.setOwner(level().getNearestEntity(DarkDoppelgangerEntity.class, TargetingConditions.forNonCombat().ignoreLineOfSight().ignoreInvisibilityTesting(), null, getX(), getY(), getZ(), this.getBoundingBox().inflate(32)));
        enderField.setPos(Utils.moveToRelativeGroundLevel(level(), this.position(), 3));
        enderField.setRadius(this.explosionRadius + 1);
        enderField.setCircular();
        enderField.setDamage(this.getDamage() * .5f);
        enderField.setDuration(20 * 15);
        enderField.setDelay(this.delay + 25);
        enderField.setRadiusPerTick(-enderField.getRadius() / enderField.getDuration());
        level().addFreshEntity(enderField);
    }

    @Override
    protected void onHitEntity(EntityHitResult entityHitResult) {
        super.onHitEntity(entityHitResult);
        entityHitResult.getEntity().hurt(new DamageSource(level().damageSources().damageTypes.getHolderOrThrow(ISSDamageTypes.ENDER_MAGIC), this, getOwner()), getDamage());
    ...
    public void createDaggerZone(Vec3 center) {
        MagicManager.spawnParticles(level(), new BlastwaveParticleOptions(SchoolRegistry.ENDER.get().getTargetingColor(), explosionRadius + 1), center.x, center.y + .15, center.z, 1, 0, 0, 0, 0, false);
        playSound(SoundRegistry.ENDER_CAST.get(), 2f, Utils.random.nextIntBetweenInclusive(80, 110) * .01f);
        ...
        for (int i = 1; i < rings; i++) {
            ...
            for (int j = 0; j < daggerCount; j++) {
                ...
                EnderDaggerEntity dagger = new EnderDaggerEntity(level());
    ...
    public void tick() {
        if (!isSpawnDagger() && age++ < delay) {
            ...
            if (level().isClientSide) {
                level().addParticle(ParticleHelper.UNSTABLE_ENDER, getX(), getY() + getBbHeight() * .5f, getZ(), 0, 0, 0);
            }
        } else {
            super.tick();
        }
    ...
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level(), ParticleTypes.DRAGON_BREATH, x, y, z, 5, .1, .1, .1, .25, true);
    }
    ...
    public void trailParticles() {
        ...
        for (int j = 0; j < steps; j++) {
            ...
            level().addParticle(ParticleHelper.UNSTABLE_ENDER, x, y, z, jitter.x, jitter.y, jitter.z);
        }
    }
    ...
    public Optional<Holder<SoundEvent>> getImpactSound() {
        return isGrounded ? Optional.empty() : Optional.of(SoundRegistry.KEEPER_SWORD_IMPACT);
    }
