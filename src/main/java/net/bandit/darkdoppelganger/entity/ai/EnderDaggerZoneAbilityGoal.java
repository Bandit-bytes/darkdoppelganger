...
public class EnderDaggerZoneAbilityGoal extends AnimatedActionGoal<DarkDoppelgangerEntity_tyros_class> {
    ...
    protected void doAction() {
        ...
        if (target != null) {
            mob.playSound(SoundRegistry.MAGIC_ARROW_RELEASE.get(), 2f, Utils.random.nextIntBetweenInclusive(80, 110) * .01f);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                EnderDaggerEntity dagger = new EnderDaggerEntity(mob.level());
                dagger.setOwner(mob);
                dagger.setPos(start);
                dagger.delay = delay;
