...
public class EnderBossAttackGoal extends GenericAnimatedWarlockAttackGoal<DarkDoppelgangerEntity> {
    ...
    protected void onHitFrame(AttackKeyframe attackKeyframe, float meleeRange) {
        if (attackKeyframe instanceof InvokeDaggerKeyframe) {
            ...
            if (attackKeyframe instanceof EnderBossAttackKeyframe enderKeyframe) {
    ...
    public void stop() {
        super.stop();
        mob.getAttribute(AttributeRegistry.CAST_TIME_REDUCTION).removeModifier(MODIFIER_ENDERIFIER);
    }
    ...
    protected void handleAttackLogic(double distanceSquared) {
        ...
        if (abilityCooldown > 0) {
            ...
            if (!mob.onGround() && distanceSquared > meleeRange * meleeRange * 2 * 2) {
                if (!isActing()) {
                    mob.getAttribute(AttributeRegistry.CAST_TIME_REDUCTION).addOrUpdateTransientModifier(MODIFIER_ENDERIFIER);
                    mob.initiateCastSpell(SpellRegistry.SONIC_BOOM_SPELL.get(), 3);
